...
- ✅ **轻量部署**：单文件二进制（前端已内嵌），仅依赖系统 FFmpeg，无需 Go / Node 运行时
- ✅ **B 站直播助手**：扫码登录 → 一键开播获取推流地址/密钥 → 自动填入输出配置（RTMP 推流到 B 站）

## 截图

![主控制台](screenshots/main.png)
...
1. **添加源**：左上「源」面板 → 添加 → 选类型（采集卡 / 图片 / 文字 / 媒体文件 / RTMP 拉流 / 纯色等）→ 填参数 → 勾选“创建后加入当前场景”
2. **布局**：画布上拖拽 / 缩放源，右侧属性面板调整位置、尺寸、裁剪、透明度；支持多场景切换
3. **预览**：底部栏「预览」启动 MJPEG 实时预览
4. **输出**：底部栏「输出」→ 配置 RTMP 推流 / 本地录制 / SRT → 启动

> 💡 **文字源**：中文渲染需要 CJK 字体（`sudo apt install fonts-noto-cjk`），程序自动检测并使用；
> 💡 **RTMP 拉流测试**：可先用公共测试流验证连通，如 `rtmp://live.hkstv.hk.lxdns.com/live/hks`；外网受限时可用局域网内自建 mediaMTX 推流再拉取。
...
- **Phase 2 ✅** Vue3 Web UI：源管理 / 场景编辑 / MJPEG 预览 / 推流控制
